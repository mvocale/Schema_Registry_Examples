...
curl -u "$api_key:$api_secret" -X POST "$endpoint_url"/subjects/transactions-avro-value/versions -H "Content-Type: application/json" --data "$(jq '. | {schema: tojson}' FinancialTransaction.avsc)"
